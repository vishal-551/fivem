...
nui('export', function() return { name = C.projectName, payload = C.snapshot() } end)
nui('import', function(data)
  assert(type(data) == 'table' and type(data.payload) == 'table', 'Invalid project import')
  C.projectId = nil; C.projectName = CineDirector.Util.safeName(data.name, 'Imported Scene'); C.restore(data.payload)
  return { name = C.projectName, entities = C.Entities.serialize() }
end)
